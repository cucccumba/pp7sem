...
{
    int nthreads, tid, i;
    //float a[N], b[N]; //arrays too big to be located in stack
...
#pragma omp section
            {
                ...
                for (i = 0; i < N; i++)
                    a[i] += DELTA * i;
                omp_unset_lock(&locka); //this thread does not use a[] anymore
                //printf("Thread %d unset lock a\n", tid);
                omp_set_lock(&lockb);
...
#pragma omp section
            {
                ...
                for (i = 0; i < N; i++)
                    b[i] += PI * i;
                omp_unset_lock(&lockb); //this thread does not use b[] anymore
                //printf("Thread %d unset lock b\n", tid);
                omp_set_lock(&locka);
